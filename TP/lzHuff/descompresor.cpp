...
FILE* crearArchivoOriginal(char *nombreArchivo) {
  ...
  nombreArchivoSalida = (char*)malloc(strlen(nombreArchivo) - 6);
  strncpy(nombreArchivoSalida, nombreArchivo, strlen(nombreArchivo) - 7);
  nombreArchivoSalida[strlen(nombreArchivo) - 7] = '\0';
...
int descomprimirLZ77(char *nombreArchivo) {
  ...
  deboQuitarRelleno = 1;
  ventana = (char*)malloc(TAM_VENTANA);
