...
void FileParser::processFile(string path)
{
	...
	while(!endFile)
	{
		...
		if (!endFile)
		{
			// Si es necesario, se intenta encontrar la titulo
			if ((index < this->lines) && (!tituloFound))
				tituloFound  = this->findtitulo(line);
	...
	// Se incorpora el texto al documento
	document->setDescription(text.toString());

	// Se setea el numero de palabras
	document->setFecha(this->getWordCount());
}
