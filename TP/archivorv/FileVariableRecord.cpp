...
int FileVariableRecord::addDocument(ByteString byteString)
{
	int filePosition = -1;
	if(file.is_open())
	{
...
void FileVariableRecord::deleteDocument(int position)
{
	int recordSize = getDocumentSize(position);

	file.seekp(position+SIZE_LONG_VAR_REC);
	this->fileFreeSpace->addFreeSpace(position,recordSize);
}

int FileVariableRecord::getDocumentSize(int position)
{
	file.seekp(position);
...
ByteString FileVariableRecord::readDocument(int position)
{
...
ByteString FileVariableRecord::readNext()
{
	...
	while (!found)
	{
		...
		else
		{
			...
			{
				bs = readDocument(this->actualPosition);
